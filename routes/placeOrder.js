...
router.post('/',  async (req, res) => {
    ...
    const lastSale = await Sale.find().sort({_id: -1}).limit(1).select('orderNo');
    ...
    .then(customer => {
        ...
        if(customer.qutyBought) customer.qutyBought = parseInt(customer.qutyBought) + parseInt(proQty)
    ...
    .then(c => {
        c.save()
        sendInvoiceMail(customerR.email, saleR, customerR, req.body.cartComplete, req.body.subtotal)
    })
    ...
    req.body.cart.forEach(p => {
        ...
        .then(product => {
            ...
            product.totalQuantity = parseInt(product.totalQuantity) - parseInt(p.qty)
            ...
            if(product.sales) product.sales = parseInt(product.sales)  + parseInt(p.qty)
            else {product.sales = parseInt(p.qty)}
